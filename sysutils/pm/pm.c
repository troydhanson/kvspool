...
#define DEFAULT_PM_CONFIG "/etc/pm.conf" 
#define SHORT_DELAY 10

char proctitle[16] = "pm";
sigjmp_buf jmp;
...
int main (int argc, char *argv[]) {
  ...
  openlog("pm", LOG_PID | (cfg.foreground ? LOG_PERROR : 0), LOG_LOCAL0);
  ...
  syslog(LOG_INFO,"pm: managing %d jobs\n", (int)utarray_len(cfg.jobs));
  snprintf(proctitle,sizeof(proctitle),"pm [%d jobs]", (int)utarray_len(cfg.jobs));
  ...
  switch(signo) {
    ...
    case SIGALRM:
      ...
      break;
    default:
      syslog(LOG_INFO,"pm: exiting on signal %d\n", signo);
      goto done;
      break;
  }
